...
//    获取文字显示�?的宽�?
    ...
    for(var i = 1;i<divs.length;i++){    //将第一个后面的div和section全部放到后面�?
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(var u = 0;u<4;u++) {
                    ...
                    var num = data[nameArray[u]].length;     //数组的个�?
                    //console.log(num);
                    var x01 = 1 / 16 * stage.width();   //设定的原坐标
                    var y01 = 2 / 5 * stage.height();    //第一条底线的纵坐�?
                    var y02 = 9 / 10 * stage.height();    //第二条底线的纵坐�?
                    var width1 = 7 / 8 * stage.width();  //设定的柱状图的总宽�?
                    var xWidth = 1 / 24 * width1;        //设定每个点之间的间距
                    var eachWidth = 1 / 6 * width1;    //每个小时的宽度为总宽度的1/6

                    var bsline1 = new Konva.Line({       //画柱状图的底�?
